...
func (s *formatSpecificTestSuite) TestSnapshotGCMinContentAgeSafety(t *testing.T) {
	...
	// Advance time close to SafetyFull.MinContentAgeSubjectToGC but with enough
	// room to preserve all the unreferenced contents created above.
	th.fakeTime.Advance(safety.MinContentAgeSubjectToGC - contentCount*time.Second - 100*time.Second)

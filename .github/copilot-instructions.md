...
Kopia is a fast and secure open-source backup/restore tool written in **Go 1.24.9** that creates encrypted snapshots and saves them to remote/cloud storage. The repository is approximately 15MB with 1,019 Go files.

**Key Technologies:**
- **Backend:** Go 1.24.9 (primary language)
...
```
**Time:** ~3-4 minutes  
**Linter:** golangci-lint v2.1.2 with timeout of 1200s  
...
**Requires:** KOPIA_PROVIDER_TEST=true, rclone binary  
...
2. **Go version mismatch:** Kopia requires Go 1.24.9 with toolchain go1.24.9 (specified in go.mod). The `go-version-file` is used in GitHub Actions.
...
- Tests marked with `// +build testing` tag
...
- Crypto: golang.org/x/crypto, zeebo/blake3
...
2. **HTML UI is separate** - The HTML UI is maintained in github.com/kopia/htmlui and imported as a pre-built module. Don't try to find UI source in this repo.

3. **Build artifacts in dist/** - Gitignored. Contains platform-specific binaries and installers after `make ci-build` or `make goreleaser`.

4. **Tools directory** - `tools/.tools/` is gitignored and populated by `make ci-setup`. Contains downloaded versions of gotestsum, linter, node, hugo, etc.
...
9. **Required tools installed by ci-setup:**
   - gotestsum 1.11.0 - Test runner with better output
   - golangci-lint 2.1.2 - Linter
   - node 22.15.1 - JavaScript runtime for app builds
   - hugo 0.113.0 - Static site generator for website

...
// ConcurrentCountMap is a concurrency-safe map from keys of type K to uint32 counters.
// It allows increments and retrievals of counts from concurrent go routines
// without additional concurrency coordination.
type ConcurrentCountMap[K comparable] struct {
	// Stores map[K]*uint32
...
func (m *ConcurrentCountMap[K]) Increment(key K) bool {
	return m.Add(key, 1)
}

// Add increases the counter for the specified key by the value of v.
// Returns true if the key already existed, false if it was newly created.
func (m *ConcurrentCountMap[K]) Add(key K, v uint32) bool {
...
func (m *ConcurrentCountMap[K]) Get(key K) (uint32, bool) {
